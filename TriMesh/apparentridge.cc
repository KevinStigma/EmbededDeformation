...
void draw_segment_app_ridge(const TriMesh *themesh,int v0, int v1, int v2,
			    float emax0, float emax1, float emax2,
			    float kmax0, float kmax1, float kmax2,
			    const vec &tmax0, const vec &tmax1, const vec &tmax2,
			    float thresh, bool to_center, bool do_test,float r, float g, float b,bool bFade)
{
...
void draw_face_app_ridges(const TriMesh *themesh,int v0, int v1, int v2,
			  const vector<float> &ndotv, const vector<float> &q1,
			  const vector<vec2> &t1, const vector<float> &Dt1q1,
			  bool do_bfcull, bool do_test, float thresh,float r, float g, float b,bool bFade)
{
#if 0
	...
	// Draw line segment
	if (!z01) {
		draw_segment_app_ridge(themesh,v1, v2, v0,
				       emax1, emax2, emax0,
				       kmax1, kmax2, kmax0,
				       tmax1, tmax2, tmax0,
				       thresh, false, do_test,r,g,b,bFade);
	} else if (!z12) {
		draw_segment_app_ridge(themesh,v2, v0, v1,
				       emax2, emax0, emax1,
				       kmax2, kmax0, kmax1,
				       tmax2, tmax0, tmax1,
				       thresh, false, do_test,r,g,b,bFade);
	} else if (!z20) {
		draw_segment_app_ridge(themesh,v0, v1, v2,
				       emax0, emax1, emax2,
				       kmax0, kmax1, kmax2,
				       tmax0, tmax1, tmax2,
				       thresh, false, do_test,r,g,b,bFade);
		...
		draw_segment_app_ridge(themesh,v1, v2, v0,
				       ...
				       kmax1, kmax2, kmax0,
				       tmax1, tmax2, tmax0,
				       thresh, true, do_test,r,g,b,bFade);
		draw_segment_app_ridge(themesh,v2, v0, v1,
				       emax2, emax0, emax1,
				       kmax2, kmax0, kmax1,
				       tmax2, tmax0, tmax1,
...
void draw_mesh_app_ridges(const TriMesh *themesh,const vector<float> &ndotv, const vector<float> &q1,
			  const vector<vec2> &t1, const vector<float> &Dt1q1,
			  bool do_bfcull, bool do_test, float thresh,float r, float g, float b,bool bFade)
{
	...
	while (1) {
		...
		draw_face_app_ridges(themesh,*(t-2), *(t-1), *t,
				     ndotv, q1, t1, Dt1q1,
				     do_bfcull, do_test, thresh,r,g,b,bFade);
		t++;
	}
}
